...
import StatusView from "@/components/tables/status-view";
import { JoinRequests } from "@/server/joinrequest";
...
const JoinRequest = () => {
  ...
  const fetchJoinRequests = useCallback(async () => {
    try {
      ...
      const res = await JoinRequests(page, currentStatus, currentSearch);
                      ...
                      <Link
                        href={
                          item.status === "Active"
                            ? `/${item?.role
                                ?.toLowerCase()
                                .replace(/_/g, "")}/${item.ext_id}`
                            : `joinrequest/${item.ext_id}`
                        }
