...
const EnterpriseTable = () => {
  ...
  const handleSelectAllChange = () => {
    if (selectAll) {
      ...
      setSelected(enterprise.map((_, index) => index)); // Select all
    }
    setSelectAll(!selectAll);
  };
  const handleDownload = () => {
    const selectedUsers = selected.map((i) => enterprise[i]);
    const dataStr =
      "data:text/json;charset=utf-8," +
      encodeURIComponent(JSON.stringify(selectedUsers));
    const downloadAnchorNode = document.createElement("a");
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", "enterprise.json");
