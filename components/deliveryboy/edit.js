...
const EditDeliveryboyPage = ({ deliveryboyId }) => {
  const { country, fetchCountry, city, state, fetchCity, fetchState } =
    useFetchGlobalData();
  ...
  const formik = useFormik({
    initialValues: {
      ...
      city:cityList?.find(
        (city) => city.value === deliveryboy?.city_id
      ) || null,
      state:stateList?.find((state)=>state.value=== deliveryboy?.state_id) || null,
