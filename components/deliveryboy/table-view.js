...
const DeliveryboyTable = () => {
  ...
  const fetchDeliveryboy = useCallback(async (currentPage, currentSearch) => {
    setLoading(true);
    try {
      const response = await GetDeliveryboys(currentPage, currentSearch);
  ...
  useEffect(() => {
    ...
    fetchDeliveryboy(page, currentSearch);
  }, [searchParams, fetchDeliveryboy]);
  ...
  const handleSelectAllChange = () => {
    if (selectAll) {
      ...
      setSelected(deliveryboy.map((_, index) => index)); // Select all
    }
    setSelectAll(!selectAll);
  };
  const handleDownload = () => {
    const selectedUsers = selected.map((i) => deliveryboy[i]);
    const dataStr =
      "data:text/json;charset=utf-8," +
      encodeURIComponent(JSON.stringify(selectedUsers));
    const downloadAnchorNode = document.createElement("a");
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute("download", "deliveryboy.json");
