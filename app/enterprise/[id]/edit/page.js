...
import { GetDetail, updateEnterprise } from "@/server/enterprise";

const EditEnterprise = ({params}) =>{
  const { country, vehicleType, city ,fetchAllData,state} = useGlobalData();
  ...
  const fetchEnterpriseData = async (id) => {
    try {
      ...
      console.log("res",res?._response[0])
    } catch (error) {
      setDeliveryboy({});
    }
  };
  ...
  useEffect(()=>{
    if(country){
      setCountryData(country?.map((item) => ({ label: item.country_name, value: item.id })));
      if(state){
        setStateList(state?.map((item) => ({ label: item.state_name, value: item.id })));
      }
      if(city){
        setCityList(city?.map((item) => ({ label: item.city_name, value: item.id })));
      }
    }else{
      fetchAllData()
    }
  ...
  const formik = useFormik({
      initialValues: {
        ...
        terms: deliveryboy?.term_cond1 ? true : false,
        termss: deliveryboy?.term_cond2 ? true : false,
      ...
      onSubmit: async (values) => {
        ...
        const payload = {
          ...
          term_cond1: values.terms === "on" ? 1 : 0,
          term_cond2: values.termss === "on" ? 1 : 0,
        ...
        try {
          ...
          setLoading(false);
          setTimeout(() =>{ setStatusMessage("");router.back()}, 2500);
        }
      },
    });
  return (
